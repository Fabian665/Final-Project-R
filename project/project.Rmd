...
# Select the relevant features and target
features <- features <- c('pentile', 'is_youth_at_risk', 'sector', 'type',
               'bagrut', 'bagrut_reg_ed', 'bagrut_excell',
              'bagrut_reg_ed_excell', 'class_size', 'median_seniority', 'masters_or_higher_in_high_school', 'hatamot_ivchun')
target <- 'math_5_reg_ed'
...
# Calculate the average math_5_reg_ed score for each pentile and class size bin
average_scores <- filtered_data %>%
  group_by(pentile, class_size_bin) %>%
