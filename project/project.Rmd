...
# Drop rows with NA values in pentile or math_5_reg_ed columns
filtered_data <- filtered_data[complete.cases(filtered_data$pentile, filtered_data$math_5_reg_ed), ]

# Calculate the average math_5_reg_ed score for each pentile and district
average_scores <- filtered_data %>%
  group_by(pentile, district) %>%
...
ggplot(merged_data, aes(x = cut(hatamot_sum, breaks = seq(0, 100, by = 10)), y = math_5_reg_ed)) +
  ...
  labs(x = "Academic Leniencies (With and Without Assessment) (%)",
       y = "5 Points In Math (%)", title = "5 Points In Math by Academic Leniencies") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
# Plot the math_5_reg_ed for each muni_name
ggplot(filtered_data, aes(x = year, y = masters_or_higher_in_high_school*100, color = muni_name, group = muni_name)) +
  geom_line(size = 1) +
  labs(x = "Year", y = "Masters Or Higher Teachers In High School (%)", title = "Masters Or Higher Teachers Over The Years, Largest Cities", color = "City") +
  theme_minimal() +
